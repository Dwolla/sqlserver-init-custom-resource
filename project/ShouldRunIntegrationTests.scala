...
object ShouldRunIntegrationTestsEitherTOps {
  private[ShouldRunIntegrationTestsEitherTOps] val noDatabaseWarning: String =
    """No SqlServer database instance was detected, so integration tests are disabled.
      |
      |You can start a local Docker container using SqlServer by running the following command:
      |
      | docker run \
      |   --detach \
      |   --name sqlserver \
