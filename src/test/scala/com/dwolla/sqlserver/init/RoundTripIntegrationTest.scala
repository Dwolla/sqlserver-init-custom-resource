...
import com.dwolla.sqlserver.init.FakeS3.CapturedRequests
import com.dwolla.sqlserver.init.aws.SecretsManagerAlg.{SecretName, SecretNamePredicate}
import com.eed3si9n.expecty.Expecty.expect
import eu.timepit.refined.refineV
import feral.lambda.cloudformation.RequestResponseStatus._
import feral.lambda.cloudformation.{CloudFormationCustomResourceArbitraries, CloudFormationCustomResourceRequest, CloudFormationCustomResourceResponse, CloudFormationRequestType, PhysicalResourceId, RequestResponseStatus}
...
import com.dwolla.sqlserver.init.aws.SecretDeletionRecoveryTime.Immediate
import com.dwolla.sqlserver.init.aws.{SecretDeletionRecoveryTime, SecretsManagerAlg}
...
class RoundTripIntegrationTest
  ...
  private val genUserConnectionInfo: Option[Database] => Gen[UserConnectionInfo] = maybeDatabase =>
    for {
      ...
      user <- arbitrary[SqlServerUser].map(Username(_))
  ...
  private def genDatabaseMetadata[F[_] : Monad, A : Encoder](secretsAlg: SecretsManagerAlg[F],
    ...
    for {
      ...
      username <- refinedConst[SqlServerUser](BuildInfo.test_databaseUser).map(MasterDatabaseUsername(_))
  ...
  private def physicalResourceIdLens[T]: Lens[CloudFormationCustomResourceRequest[T], Option[PhysicalResourceId]] = GenLens[CloudFormationCustomResourceRequest[T]](_.PhysicalResourceId)
  ...
  private implicit val logHandler: LogHandler = LogHandler { event =>
    println(s"🔮 ${AnsiColorCodes.red}${event.sql}${AnsiColorCodes.reset}")
  }
    ...
    PropF.forAllF { (input: Resource[IO, CloudFormationCustomResourceRequest[DatabaseMetadata]]) =>
      val testResources =
        for {
          ...
          handler <- new SqlServerDatabaseInitHandlerF[IO] {
        ...
        } yield (requestCapture, { (requestType: CloudFormationRequestType) =>
          requestTemplate
            .applyLens(requestTypeLens)
            .set(requestType)
            .applyLens(physicalResourceIdLens)
            .modify(_.filterNot(_ => requestType == CreateRequest))
        }, handler, secrets)
  ...
  def expectReservedIdentifiersError: CapturedRequests[IO] => IO[Unit] =
    expectErrorMessageStartsWith("The specified secrets refer to usernames that are SqlServer reserved words. Change or remove those users from the input and try again.")
